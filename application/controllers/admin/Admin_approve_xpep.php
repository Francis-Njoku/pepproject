...
class Admin_approve_xpep extends CI_Controller {
    ...
    public function index($offset = 0) {
        ...
        $limit = 10;
        $result = $this->transaction->get_blogs2($limit, $offset);
        // $data['blog_list'] = $result['rows'];
        //$data['num_results'] = $result['num_rows'];
        $data['blog_list'] = $this->transaction->get_blogs2($limit, $offset);
        $data['num_results'] = $this->transaction->result2();

        // load pagination library
        $config = array();
        $config['base_url'] = site_url().'/admin/admin_approve_xpep/index';
        ...
        $this->load->view('admin/admin_approve_xpep_view', $data);
    ...
    public function credit()
    {
        ...
        {
            $xpep_address = $user_details['xpep_wallet_address'];

        }

        # calculate amount in PEP
        $xpep_amount = $currency_value;
        $xpep_credit = $xpep_amount * $amount;
        ...
        if (isset($this->session->userdata['user_id'])) {
            ...
            else
            {
                ...
                redirect(site_url('/admin/admin_approve_xpep'));
            }
        }
        else
        {
