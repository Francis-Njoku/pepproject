...
class Fund_wallet extends CI_Controller {
	...
	public function save_order()
	{
		...
		$user_payment = array(
            ...
            'currency_to' 		=> 'PEP',
            'amount' 		=> $this->input->post('value_in_pep'),
        ...
        redirect(site_url('frontend/fund_wallet'));
	}
    ...
    public function save_order2()
    {
        ...
        $from = $this->input->post('destination');

        $user_payment = array(
            'currency_from' 		=> $this->input->post('currency'),
            'currency_to' 		=> $this->input->post('destination'),
            'amount' 		=> $this->input->post('value_in_pep'),
            'deposit' 		=> $this->input->post('amount'),
            'referrence'	=> $this->input->post('address').' from '.$from,
            'status' 		=> $status,
            'user_id' 		=> $user_id,
            'date' 		=> $deig
        );
        ...
        $this->session->set_flashdata('flash_message', 'Successful');
        redirect(site_url('frontend/dashboard/pep'));
    }

    public function save_order3()
    {
        if (isset($this->session->userdata['user_id'])) {
        ...
        $user_payment = array(
            'currency_from' 		=> $this->input->post('currency'),
            'currency_to' 		=> $this->input->post('currency'),
        ...
        $this->session->set_flashdata('flash_message', 'Successful');
        redirect(site_url('frontend/fund_wallet_history/'));
    }
